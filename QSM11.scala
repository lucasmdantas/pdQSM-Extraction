...
Q.saveAsTextFile(args(2)+"/QSM/Q",classOf[org.apache.hadoop.io.compress.GzipCodec])

//Q foreach println

//Saving number of times(Quantity) byte ocurred in each block
val S = QS.map(record => record._2)
S.saveAsTextFile(args(3)+"/QSM/S",classOf[org.apache.hadoop.io.compress.GzipCodec])
